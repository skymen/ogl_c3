(function () {
    ...
    computeBoundingSphere(attr) {
      ...
      for (let i = 0, l = array.length; i < l; i += stride) {
        ...
        maxRadiusSq = Math.max(
          maxRadiusSq,
          this.bounds.center.squaredDistance(tempVec3$6)
        );
      }
  ...
  class Renderer {
    ...
    getExtension(extension, webgl2Func, extFunc) {
      ...
      return this.extensions[extension][extFunc].bind(
  ...
  class Transform {
    ...
    updateMatrix(force = false) {
      if (
        !this.oldPosition.equals(this.position) ||
        !this.oldScale.equals(this.scale)
      ) {
        this.matrixNeedsUpdate = true;
  ...
  class Texture {
    ...
    update(textureUnit = 0) {
      const needsUpdate = !(
        this.image === this.store.image && !this.needsUpdate
      );
    ...
    constructor(
      ...
      function addHeight() {
        ...
        for (y = 0; y <= hSegs; y++) {
          ...
          for (x = 0; x <= rSegs; x++) {
            ...
            position.set(
              [r * sinTheta, (0.5 - v) * height, r * cosTheta],
              i * 3
            );
      ...
      function addCap(isTop) {
        ...
        for (x = 0; x <= rSegs; x++) {
          ...
          position.set(
            [r * sinTheta, 0.5 * height * sign, r * cosTheta],
            i * 3
          );
          normal.set([0, sign, 0], i * 3);
  ...
  class Raycast {
    ...
    castMouse(camera, mouse = [0, 0]) {
      if (camera.type === "orthographic") {
        ...
        const x =
          left / zoom + ((right - left) / zoom) * (mouse[0] * 0.5 + 0.5);
        const y =
    ...
    findSegmentIndexAtLength(len) {
      ...
      while (start <= end) {
        ...
        if (
          ...
          (len >= this._lengthOffsets[mid] &&
            len < this._lengthOffsets[mid + 1])
        ) {
          index = mid;
          break;
...
`;
  ...
  class GLTFLoader {
    ...
    static parseLights(gl, desc, nodes, scenes) {
      ...
      const lightsDescArray =
        desc.extensions?.KHR_lights_punctual?.lights || [];
...
`;
  ...
  var OGL = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    Animation: Animation,
  ...
  // define OGL as a global variable
  const scope = globalThis || self || window;
  scope.OGL = OGL;
})();
